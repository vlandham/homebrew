...
private
  ...
  def mktemp
    ...
    tmp=Pathname.new `#{OS.mktemp} -d #{tmp_prefix}/homebrew-#{name}-#{version}-XXXX`.strip
...
EOF
  ...
  def patch
    ...
    patch_list.each do |p|
      case p[:compression]
        when :gzip  then safe_system OS.gunzip,  p[:filename]+'.gz'
        when :bzip2 then safe_system OS.bunzip2, p[:filename]+'.bz2'
      end
      # -f means it doesn't prompt the user if there are errors, if just
      # exits with non-zero status
      safe_system OS.patch, '-f', *(p[:args])
    end
  end
